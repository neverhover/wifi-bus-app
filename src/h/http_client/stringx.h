...
*/
/* added by zhangly */
char* http_client_base64_decode(char *b64src) 
{
	char *clrdst = (char*)malloc( ((strlen(b64src) - 1) / 3 ) * 4 + 4 + 50);
	if(clrdst == NULL){
		return NULL;
	}
	memset(clrdst, 0,  ((strlen(b64src) - 1) / 3 ) * 4 + 4 + 50 );
...
*/
#if 1
/* added by zhangly,because goahead had defined this function */
char* http_client_base64_encode(char *clrstr) 
{
	char *b64dst = (char*)malloc(strlen(clrstr) + 50);
	if(b64dst == NULL){
		return NULL;
	}
	memset(b64dst, 0, strlen(clrstr) + 50);
	char b64[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	unsigned char in[3];
	int i, len = 0;
	int j = 0;
