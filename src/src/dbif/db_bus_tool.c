...
int db_open(char *db_path){
	...
	if(semid < 0){
		...
		DB_STRACE("semid alreay exist\n");
		semid = semget(key, 1,  0666);
	}else{
		DB_STRACE("new semid init it\n");
...
int db_get_search_count(sqlite3 *db, const char *table_name, char *search_str , int *row_count){
	...
	if(NULL == db || NULL == table_name  ||NULL== search_str || NULL == row_count ){
		fprintf(stderr, "SQL input is error db_get_search_count\n");
		return ERROR;
	}
	ret = db_construct_dbstr(&pStmt, db, SELECT_COUNT_FMT, table_name, search_str);
	if(ERROR == ret){
		fprintf(stderr, "SQL db_construct_dbstr error\n");
		return ERROR;
	}
	//执行语句
 ...
 * 删除表的所有记录
 */
int db_delete_table_rows(sqlite3 *db, const char *table_name, char *condition_str , int *row_count){
	int ret;
	sqlite3_stmt *pStmt = NULL;
	...
	ret = db_construct_dbstr(&pStmt, db, DELETE_ALL_FMT, table_name, condition_str);
	if(ERROR == ret){
		fprintf(stderr, "SQL db_construct_dbstr error\n");
		return ERROR;
	}
